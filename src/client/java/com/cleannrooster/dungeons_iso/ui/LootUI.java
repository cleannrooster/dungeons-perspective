...
public class LootUI extends BaseOwoScreen<FlowLayout> {
    ...
    protected void build(FlowLayout rootComponent) {
        ...
        if(Minecraft.getInstance().player != null) {
            List<ItemEntity> itemEntityList = Minecraft.getInstance().player.getWorld().getEntitiesByType(TypeFilter.instanceOf(ItemEntity.class),Minecraft.getInstance().player.getBoundingBox().expand(16),(entity) ->{
                return Minecraft.getInstance().player.canSee(entity);
            });
            int ii = 0;
                ...
                if(ii < 8) {
                    ...
                    verticalList.add(
                            Containers.horizontalFlow(Sizing.content(),Sizing.content())  .child(Components.button(entity.getName(), button -> {
                                ((MinecraftAccessor)Minecraft.getInstance()).setLocation(new EntityHitResult(entity,entity.getPos()));
                                ((MinecraftAccessor)Minecraft.getInstance()).setOriginalLocation(Minecraft.getInstance().player.getPos());
